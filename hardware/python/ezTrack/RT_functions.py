...
class Video:
    ...
    def ref_create(self, display_count=False, secs=5):
            ...
            if krn_violation:
                print(
                    "WARNING: wire_krn too large. Reverting to rmv_wire=False for frame {x}".format(
                        x=int(
                            cap.get(cv2.CAP_PROP_POS_FRAMES) - 1 - video_dict["start"]
                        )
                    )
                )
...
class Saver:
    def __init__(self, vid, dpath, bufsize=300):
        ...
        self.buffer = np.zeros((self.scale.h, self.scale.w, self.bufsize)).astype(
            "uint8"
        )
    ...
    def start(self):
        ...
        multiprocessing.Process(
            ...
            args=(self.vpath, self.bufqueue, self.vidstartlen, self.stopsig),
        ).start()
        ...
        with h5py.File(name=vpath, mode="r+") as vfile:
            ...
            while not stopsig.is_set():
                try:
                    ...
                    if fnums[1] >= filelen:
                        filelen += 1000
                        video.resize((buffer.shape[0], buffer.shape[1], filelen))
